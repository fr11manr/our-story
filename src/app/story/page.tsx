...
import { LoadingMemoryGrid, useStoryAssets, useStoryChapters } from "@/components/MemoryCards";
import { PageTransition } from "@/components/PageTransition";
import { SectionHeader } from "@/components/SectionHeader";
import { SiteShell } from "@/components/SiteShell";

export default function StoryPage() {
  const { items: assets, loaded: assetsLoaded } = useStoryAssets();
  const { items: chapters, loaded: chaptersLoaded } = useStoryChapters();
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
          ...
          <article className="rounded-[34px] border border-white/10 bg-white/[0.07] p-6 backdrop-blur-xl sm:p-9">
            <div className="space-y-8">
              {!chaptersLoaded ? <LoadingMemoryGrid count={3} /> : chapters.map((chapter) => (
